function [ripK,mechs,egrow,images] = JC_holdAbund2(pred,predSeedling,bint,...
   yield,dDist,death,TIME1,TIME2,LEN,IVAL,SHAKE,INVADE,SNAP)


%function [ripK,mechs,egrow,images] = JC_holdAbund2(pred,predSeedling,bint,...
%   yield,dDist,death,TIME1,TIME2,LEN,IVAL,SHAKE,INVADE,SNAP)
%
%This function runs the simulation in which the abundance of one species is held constant.    It is nearly identical to JC_invade1.  The function takes the following input variables:
%
...
%IVAL- This is the number of invaders that the focal population is held at.
...
%INVADE- This is the species number of the focal population.
%SNAP- This is a 1xn matrix.  The function outputs the distribution of the population at each time in SNAP.
%
...
%images- This is an AREAxN matrix.  It records the distribution of individuals at the N times indicated in SNAP.
%
...
if(length(LEN)==1)
    ...
    AREA=LEN^2;
    N=ceil([1:AREA]*SPP/AREA);
    
    %Here I increase the focal population's abundance to IVAL
    while(sum(N==INVADE)<IVAL)
        N(randi(AREA))=INVADE;
    end
    N=shuffle(N);
else
...
for tt=2:TIME
    ...
    %Here I record the distribution of all individuals
    if(sum(tt==SNAP)>0)
        here=sum((tt==SNAP).*[1:length(SNAP)])
        images(:,here)=N;
    end
    
    %Here I record the location of the focal species, in case they need to be added back in later.
    invhere=(N==INVADE).*[1:AREA];
    invhere(invhere==0)=[];
    ...
    %%%%%%%%%%%%%%%
    %add or remove the focal species
    
    invnum=sum(N==INVADE);
    
    Igrow(1,tt-1)=invnum/IVAL;

    %If the focal population has grown, then I kill some off randomly.
    if(invnum>IVAL)
        
        %First I find all of the locations where the focal species is.
        invhere=(N==INVADE).*[1:AREA];
        invhere(invhere==0)=[];
        toRemove=shuffle(invhere);
        
        %Then I randomly chose who will replace them.
        temp=randi(SPP-1,1,invnum-IVAL);
        temp(temp>=INVADE)=temp(temp>=INVADE)+1;
        
        %Then I replace them
        N(toRemove(1:(invnum-IVAL)))=temp;
        
    %If the focal population has shrunk, then I replace some of the ones that died.
    elseif(invnum<IVAL)
        %This is where the focal pop was at the start of the time step.
        invhere=shuffle(invhere);
        i=1;
        
        %I cycle through all sites where the focal pop was at the start of the time step.  If they died, I replace them.  I do this until the focal population's abundance is back to IVAL.
        while(sum(N==INVADE)<IVAL)
        if(i>length(invhere))
            i
            length(invhere)
